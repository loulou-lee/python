# 정규화(regularized) 선형회귀 방법은 선형회귀 계수(weight)에 대한 제약 조건을 추가함으로써 모형이 과도하게 최적화되는 현상, 
# 즉 과최적화를 막는 방법이다. Regularized Method, Penalized Method, Contrained Least Squares 이라고도 불리운다.

# linear regression 모형이 과도하게 최적화되면(overfitting) 모형 계수의 크기도 과도하게 증가하는 경향이 나타난다. 
# 따라서 정규화 방법에서 추가하는 제약 조건은 일반적으로 계수의 크기를 제한하는 방법이다. 일반적으로 다음과 같은 세가지 방법이 사용된다.
# Ridge 회귀모형
# Lasso 회귀모형
# Elastic Net 회귀모형
